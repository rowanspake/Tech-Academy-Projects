...
#catch if user clicks on upper-right 'X'
def ask_quit(self):
...
def addToList(self):
    ...
    var_fname = var_fname.strip() # This will remove any black spaces before and after the user's entry
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0): # make sure user provides both names
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
...
def onDelete(self):
    ...
    with conn:
        cur = conn.cursor()
        # check count to ensure that this is not the last record in db; last record will throw error on delete
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            if count == 0 or count2 == 0: # if proposed changes are not already in the database, then proceed
                response = messagebox.askokcancel("Update Request","The following changes ({}) and ({}) will be implemeneted for ({}). \n\nProceed with the update request?".format(var_phone,var_email,var_value))
                print(response)
                if response:
                    #conn = sqlite3.connect('db_phonebook.db")
                    with conn:
