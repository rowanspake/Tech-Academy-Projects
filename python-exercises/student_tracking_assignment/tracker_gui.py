...
def load_gui(self):
    ...
    self.lbl_info.grid(row=0,column=2,columnspan=2,padx=(0,0),pady=(10,0),sticky=N+W)
    ...
    self.scrollbar1.grid(row=1,column=2,rowspan=7,columnspan=1,padx=(0,0),pady=(0,0),sticky=N+E+S)
    self.lstList1.grid(row=1,column=2,rowspan=7,columnspan=3,padx=(0,0),pady=(0,0),sticky=N+E+S+W)

    self.btn_add = tk.Button(self.master,width=12,height=2,text='Add',command=lambda: tracker_func.addToList(self))
    self.btn_add.grid(row=10,column=0,padx=(25,0),pady=(45,10),sticky=W)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: tracker_func.onDelete(self))
    self.btn_delete.grid(row=10,column=1,padx=(15,0),pady=(45,10),sticky=W)
    self.btn_close = tk.Button(self.master,width=12,height=2,text='Close',command=lambda: tracker_func.ask_quit(self))
    self.btn_close.grid(row=10,column=2,padx=(15,0),pady=(45,10),sticky=W)
